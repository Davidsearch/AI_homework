...
model_dir = os.path.join(current_dir, 'blip-image-captioning-base')  # 模型目录
...
for epoch in range(num_epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        early_stop_counter = 0
        # 保存最佳模型
        model.save_pretrained(os.path.join(current_dir, "best_model"))
        processor.save_pretrained(os.path.join(current_dir, "best_model"))
    else:
